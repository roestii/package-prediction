...
def main():
    ...
    result = differential_evolution(lgbm_fitness, len(bounds), bounds, cr=0.8, f=0.9)
    df = pd.DataFrame(result, columns=["learning_rate", "num_leaves", "max_depth", "min_data_in_leaf", "num_boost_round"])
    df.to_csv("result.csv", index=False)
    dataset = Dataset()
    best_model = model_from_vector(result[0], dataset.x_train, dataset.y_train)
...
def differential_evolution(
    ...
    for k in range(iterations):
        ...
        intermediate_result.to_csv(f"./intermediate_results/intermediate_result_{k}.csv", index=False)
        os.system("clear")
